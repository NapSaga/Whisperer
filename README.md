...
- **Cost — the important one.** What's real and measured: the Whisperer side is genuinely lighter because it sends compact state instead of resending history. The *magnitude* of the saving (e.g. 7.6×) is a **projection** from official Realtime rates + our StudierAI numbers (€4.50 vs €0.13) — the on-screen counter is labeled **"projection"**, never "measured", and the mock `cost.json` is never passed off as a measurement. On a short 28-turn demo the dramatic divergence does not exist (forgetting and cost-magnitude are coupled, and only both hold on a long call); on a real 10–20 min call the gap compounds.

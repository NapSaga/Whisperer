...
## Avanzamento — Fatto 8 / Totale 37 (22%)
```
████░░░░░░░░░░░░░░░░░░  22%
```
...
## Fase 1 — web/ mock-first → checkpoint 13:00 · owner: **Giovanni** · 0/11
> Gira **interamente** su `spec/fixtures/` finché il server non c'è. Grande, leggibile da proiettore, dark-only.
- `[todo]` **App Next.js** in `web/` — single page, App Router, dark-only · *(Context7: Next.js)*
- `[todo]` **shadcn init + add** (`card badge button table tabs separator progress skeleton sonner scroll-area collapsible`) + **ai-elements** (`conversation message response`) · *(shadcn CLI/MCP)*
- `[todo]` **Design tokens doc 7**: bg `zinc-950→900` · surface `zinc-900`+`white/10` · **PASS `emerald-500`+glow** · FAIL `red-500` · RUNNING `amber-400` · accent voce `violet-400` · Geist Sans (UI) / Geist Mono (dati, `[t{n}]`, timestamp) · radius `0.75rem`
- `[todo]` **Fixtures loader tipizzato** sui contratti `SPEC §7` (normalizza `t41_base`→sx, `t41_suggeritore`→dx come `t41`) — **← transcript turn · state.json · cost_event · verdict**
- `[todo]` **Replay driver**: timeline sul campo `ts` (minuto 0→10), play/pause, salto al momento `t38→t41`
- `[todo]` **Split-screen** base (sx) vs suggeritore (dx) — transcript con ai-elements `conversation`/`message` — **← transcript turn**
- `[todo]` **Memoria HUD live**: `state.json` riga per riga, ogni fatto/impegno col suo **`[t{n}]`** accanto, che si scrive mentre la timeline scorre — **← state.json**
- `[todo]` **Cost counter divergente**: base sale ripido, suggeritore quasi piatto, `usd_cumulative` a schermo — **← cost_event**
